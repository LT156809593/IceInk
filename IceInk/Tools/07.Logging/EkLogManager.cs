...
namespace IceInk
{
    ...
    public static class EkLogManager
    {
        ...
        static EkLogManager()
        {
            var writeTask = new Task(obj =>
            {
                ...
                while (true)
                {
                    ...
                    //日志写入
                    foreach (var item in temp) WriteText(item[0], item[1]);
                }
        ...
        private static string GetLogPath()
        {
            ...
            var extension = ".log"; //文件后缀 .log
            var fileNameNotExt = Now.ToString("yyyyMMdd"); //文件名称 当天
            ...
            {
                var fileMaxLen = filePaths.Max(d => d.Length); //获取日志文件的最长文件名，
                var lastFilePath = filePaths.Where(d => d.Length == fileMaxLen)
                ...
                if (new FileInfo(lastFilePath).Length > logFileInfoLen) //如果超出规定大小，则添加一个新的日志文件
                {
                    ...
                    var parse = int.TryParse(no, out var tempno); //转换日志文件编号为int
                    var formatno = $"({(parse ? tempno + 1 : tempno)})"; //日志文件编号增加1
