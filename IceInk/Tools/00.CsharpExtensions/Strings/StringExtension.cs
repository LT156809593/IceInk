...
namespace IceInk
{
    ...
    public static class StringExtension
    {
        ...
        public static Match IsPhoneNumber(this string s, out bool isMatch)
        {
            ...
            var match = Regex.Match(s, @"^((1[3,4,5,6,7,8,9][0-9])|(14[5,7])|(17[0,1,3,6,7,8])|(19[8,9]))\d{8}$");
        ...
        private const string CarNumRegex = @"([京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}(([0-9]{5}[DF])|([DF]([A-HJ-NP-Z0-9])[0-9]{4})))|([京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-HJ-NP-Z0-9]{4}[A-HJ-NP-Z0-9学警港澳]{1})";
        /// <summary>
        /// 正则表达式验证车牌号,新能源车牌号
        /// </summary>
        /// <param name="carNo">车号</param>
        /// <returns>true:验证成功，false:验证失败</returns>
        public static bool IsVehicleNumber(this string carNo)
        {
            string car = carNo.ToUpper();
            //Regex regex=   new Regex(@"^(([\u4e00-\u9fa5][a-zA-Z]|[\u4e00-\u9fa5]{2}\d{2}|[\u4e00-\u9fa5]{2}[a-zA-Z])[-]?|([wW][Jj][\u4e00-\u9fa5]{1}[-]?)|([a-zA-Z]{2}))([A-Za-z0-9]{5}|[DdFf][A-HJ-NP-Za-hj-np-z0-9][0-9]{4}|[0-9]{5}[DdFf])$", RegexOptions.Compiled);

            //return  regex.IsMatch(car);

            var result = Regex.IsMatch(car, CarNumRegex);
            return result;
        }
