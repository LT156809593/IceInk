...
namespace IceInk
{
    ...
    public class NumberFormater
    {
        ...
        public NumberFormater(int bin)
        {
            ...
            if (bin > 64) throw new ArgumentException("默认进制最大支持64进制");
        ...
        {
            var j = 0;
            return new string(str.ToCharArray().Reverse().ToArray()).Where(ch => Characters.Contains(ch))
                .Sum(ch => Characters.IndexOf(ch) * (long) Math.Pow(Length, j++));
        }
        ...
        public BigInteger FromStringBig(string str)
        {
            ...
            return chars.Aggregate(BigInteger.Zero,
                (current, c) => current + Characters.IndexOf(c) * BigInteger.Pow(Length, j++));
        }
        ...
        private static string ChangeInt(string x)
        {
            ...
            while ((i = ret.IndexOf("零零", StringComparison.Ordinal)) != -1) ret = ret.Remove(i, 1);
        ...
        private static string ToInt(string x)
        {
            ...
            while ((i = result.IndexOf("零零", StringComparison.Ordinal)) != -1) result = result.Remove(i, 1);
        ...
        public static string ToChineseNumber(double num)
        {
            ...
            if (x[0].ToString() == ".") x = "0" + x;

            if (x[x.Length - 1].ToString() == ".") x = x.Remove(x.Length - 1, 1);

            if (x.IndexOf(".") > -1)
                result += ToInt(x.Substring(0, x.IndexOf("."))) + "点" + x.Substring(x.IndexOf(".") + 1)
