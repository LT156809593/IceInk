...
namespace IceInk
{
    ...
    public class NumberFormater
    {
        ...
        {
            int j = 0;
            return new string(str.ToCharArray().Reverse().ToArray()).Where(ch => this.Characters.Contains(ch))
                .Sum(ch => this.Characters.IndexOf(ch) * (long) Math.Pow(this.Length, j++));
        }
        ...
        public BigInteger FromStringBig(string str)
        {
            ...
            return chars.Aggregate(BigInteger.Zero,
                (current, c) => current + this.Characters.IndexOf(c) * BigInteger.Pow(this.Length, j++));
        }
