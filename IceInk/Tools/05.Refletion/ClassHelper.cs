...
namespace IceInk
{
    ...
    public static class ClassHelper
    {
        ...
        public static Type BuildType(string className)
        {
            ...
            var myAsmBuilder = AssemblyBuilder.DefineDynamicAssembly(myAsmName, AssemblyBuilderAccess.RunAndCollect);
        ...
        public static object AddProperty(this object obj, List<CustPropertyInfo> lcpi)
        {
            ...
            foreach (var cpi in customs) dest.SetProperty(cpi.Key, cpi.Value);
        ...
        public static object DeleteProperty(this object obj, List<string> propertyNames)
        {
            ...
            foreach (var p in newInstance.GetProperties())
                newInstance.SetProperty(p.Name,
                    obj.GetType().GetProperties().FirstOrDefault(i => i.Name.Equals(p.Name)).GetValue(obj));
        ...
        private static void AddPropertyToTypeBuilder(TypeBuilder myTypeBuilder, List<CustPropertyInfo> lcpi)
        {
            ...
            foreach (var cpi in lcpi)
            {
                ...
                var custNamePropBldr =
                    myTypeBuilder.DefineProperty(cpi.PropertyName, PropertyAttributes.HasDefault, cpi.Type, null);

                //定义Get方法。
                var custNameGetPropMthdBldr =
                    myTypeBuilder.DefineMethod(cpi.GetPropertyMethodName, getSetAttr, cpi.Type, Type.EmptyTypes);
                ...
                //定义Set方法。
                var custNameSetPropMthdBldr = myTypeBuilder.DefineMethod(cpi.SetPropertyMethodName, getSetAttr, null,
                    new[]
                    {
                        cpi.Type
                    });
